...
export async function GET(
  ...
  await execute('UPDATE products SET view_count = view_count + 1 WHERE id = ?', params.id);

  const images = await queryOne<any[]>(
...
export async function PUT(
  ...
  const product = await queryOne<{ seller_id: number }>('SELECT seller_id FROM products WHERE id = ?', params.id);
...
export async function DELETE(
  ...
  const images = await queryOne<any[]>('SELECT image_path FROM product_images WHERE product_id = ?', params.id);
  if (images) {
